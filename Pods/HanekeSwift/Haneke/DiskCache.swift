...
open class DiskCache {
    ...
    open func setData(@autoclosure(escaping) _ getData: () -> Data?, key: String) {
    ...
    open func fetchData(key: String, failure fail: ((NSError?) -> ())? = nil, success succeed: (Data) -> ()) {
        cacheQueue.async {
    ...
    open func updateAccessDate(@autoclosure(escaping) _ getData: () -> Data?, key: String) {
        cacheQueue.async(execute: {
            let path = self.pathForKey(key)
            let fileManager = FileManager.default
