...
open class NetworkFetcher<T : DataConvertible> : Fetcher<T> {
    ...
    open override func fetch(failure fail : ((NSError?) -> ()), success succeed : (T.Result) -> ()) {
    ...
    fileprivate func onReceiveData(_ data: Data!, response: URLResponse!, error: NSError!, failure fail: ((NSError?) -> ()), success succeed: (T.Result) -> ()) {
    ...
    fileprivate func failWithCode(_ code: HanekeGlobals.NetworkFetcher.ErrorCode, localizedDescription: String, failure fail: ((NSError?) -> ())) {
